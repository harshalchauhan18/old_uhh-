...
!-----------------------------------------------------------------------
   ...
   real(rk)     :: rkc=0.07  
   real(rk)     :: sr=0.0625 
   real(rk)     :: s2=6.625 !oxygen demand/recycled nitrigen
   real(rk)     :: s3=8.625 !oxygen production related to N
   real(rk)     :: mdt=1800.
  ...
  namelist /uhh_diatoms/ veg_initial,res_initial,     &
                      mumax,vmort,rmort,rkn,rdepo,sr,s2,s3,   &
   ...
   call self%register_state_variable(self%id_veg,'veg', &
         'mmol n/m**3','diatoms vegetative biomass', veg_initial, &
         minimum=0.0e-7_rk,vertical_movement=self%w_dia)

   call self%register_state_variable(self%id_res,'res', &
         'mmol n/m**3','vegetatives resting biomass', res_initial, &
         minimum=0.0e-7_rk,vertical_movement=self%w_dia)
