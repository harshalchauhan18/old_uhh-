...
!-----------------------------------------------------------------------
   ...
   real(rk)     :: veg_initial=0.1
   real(rk)     :: gam_initial=0.0001
   real(rk)     :: res_initial=0.0001
   real(rk)     :: ger_initial=0.0001
  ...
  namelist /uhh_dinoflag/ veg_initial,gam_initial,     &
                      res_initial,ger_initial,mumax,vmort,     &
                      rkn,rdepo, trate_veg_gam,trate_gam_res,  &
                      trate_res_ger,trate_ger_veg,gr_crit,     &
                      w_veg,w_gam,w_res,w_ger,rkc,alpha_dfl,   &
                      l_crit,tmat_crit,tmat_initial,mdt,sr,    &
...
99 call self%fatal_error('fabm_uhh_dinoflag','Error reading namelist uhh_dinoflag')
   ...
   ! res -> ger
   ! note since April 2015: _VARIABLE_VERT_LOOP_ got removed and vectorization
   ! in FABM can be implemented for different indices. The index-macro _I_
   ! will denote a vertical index.
   ! already working condition:
   if (self%use_doy_for_maturation) then
     res_ger_maturation = doy .lt. self%doy_max
   else
     res_ger_maturation = tmat .gt. self%tmat_crit
   end if

   if (res_ger_maturation .and. (_I_.eq.1)) &
   ...
   _SET_ODE_(self%id_ger,-ger*self%vmort - ger*tau_ger_veg + res*tau_res_ger)
   _SET_ODE_(self%id_res,gam*tau_gam_res - res*tau_res_ger)
   ...
   _SET_ODE_BEN_(self%bsv_rsum,rmax*one_pr_day)
