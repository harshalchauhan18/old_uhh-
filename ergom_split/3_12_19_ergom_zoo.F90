...
! !LOCAL VARIABLES:
   ...
   namelist /uhh_ergom_split_zoo/ &
             ...
             gmax_dia, gmax_fla, gmax_din, gmax_cya, p_to_n, s2, &
...
!
! !LOCAL VARIABLES:
   real(rk)                   :: dia,fla,din,cya,zoo,temp
...
!BOC
   ...
   psum = dia + fla + din + cya + self%zoo0
   ...
   _SET_ODE_(self%id_zoo,self%slopf*(graz_dia + graz_fla + graz_din + graz_cya) - mortality - excretion)
   _SET_ODE_(self%id_ammonium,excretion)
   _SET_ODE_(self%id_detritus,mortality + (1.0_rk-self%slopf)*(graz_dia + graz_fla + graz_din + graz_cya))
   ...
   _SET_DIAGNOSTIC_(self%id_secprod_total,(self%slopf*(graz_dia + graz_fla + graz_din + graz_cya))*secs_pr_day)
...
!BOC
   ...
   _SET_DD_SYM_(self%id_zoo,self%id_detritus,mortality + (1.0_rk-self%slopf)*(graz_cya + graz_din + graz_fla + graz_dia))
